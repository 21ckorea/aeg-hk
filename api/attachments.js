...
module.exports = async (request, response) => {
  ...
  try {
    ...
    const { diaryId, fileName, contentType, data } = typeof request.body === 'string' ? JSON.parse(request.body) : request.body || {};
    if (!diaryId || !fileName || !data) return response.status(400).json({ error: '업무일지와 파일이 필요합니다.' });
    const bytes = Buffer.from(data, 'base64');
    if (!bytes.length || bytes.length > 4 * 1024 * 1024) return response.status(413).json({ error: '첨부파일은 현재 4MB 이하만 업로드할 수 있습니다.' });
    ...
    const storagePath = `${nextcloud.basePath}/${diaryId}/${crypto.randomUUID()}-${safeName(fileName)}`;
    ...
    await sql.query('INSERT INTO public.diary_attachments (id, diary_id, uploader_id, file_name, content_type, byte_size, storage_path) VALUES ($1,$2,$3,$4,$5,$6,$7)', [id, diaryId, user.id, safeName(fileName), contentType || null, bytes.length, storagePath]);
    return response.status(201).json({ attachment: { id, fileName: safeName(fileName), byteSize: bytes.length } });
  } catch (error) {
